...
namespace {
	void precompute_slider(slider_data& sl, std::vector<v2f>& positions, 
		...
		sl.pos_at_ms.push_back(sl.points[0]);

		for (size_t k = 2; k < positions.size() - 1; k++) {
		...
		sl.pos_at_ms.push_back(sl.points[sl.points.size() - 1]);
	}

	bool is_straight_line(const v2f& a, const v2f& b, const v2f& c) {
		return (b.x - a.x) * (c.y - a.y) - (c.x - a.x) * (b.y - a.y) == 0.0f;
		// TODO: check whether this needs an epsilon comparison or not
		// (probably not)
	}
}
	...
	switch (sl.type) {
		...
		linear_case:
			...
			if (sl.points.size() > 2) {
				...
				// TODO: don't do this as it's way more expensive
		...
		case 'P':
			if (sl.points.size() < 2) {
				die("Found perfect slider with invalid point count");
				return v2f{0};
			}
			if (sl.points.size() < 3) {
				sl.type = 'L';
			...
			if (sl.points.size() > 3) {
				sl.type = 'B';
				goto do_precomputed_curve;
				// apparently there's 2-point perfect sliders... wtf?
			}
			if (is_straight_line(sl.points[0], sl.points[1], sl.points[2])) {
				sl.type = 'L';
				goto linear_case; // TODO: proper linear slider logic so we 
				// don't need to convert to bezier
			}
			goto do_precomputed_curve;
...
do_precomputed_curve:
			...
			f64 px_per_ms = sl.length / ((f64)duration / (f64)sl.repetitions);
			...
			if (sl.type != 'B') {
				...
				for (size_t j = 1; j < sl.points.size(); j++) {
			...
			while (sl.pos_at_ms.size() < (size_t)duration + 1) {
